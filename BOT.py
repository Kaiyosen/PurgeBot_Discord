...
async def Report(ctx, member:discord.Member):
    ...
    if not messages:
        ...
        toxicity ="Toxic" if (predictions.sum().item()/len(messages) <= 0.5) else "Non-Toxic"
        if toxicity=="Toxic":
            existing_user = messages_collection.find_one({"author_name": member.name})
            if existing_user:
                messages_collection.update_one(
                {"author_name": member.name},{"$set": {"messages": messages},"$inc": {"counter": 1}}
                )
            else:
                messages_collection.insert_one(
                    {"author_name": member.name,"messages": messages,"counter": 1}
                )
        await ctx.send("\nYour report has been acknowledged!\n")
    counter_check = messages_collection.find_one({"author_name": member.name})
    if counter_check is not None and counter_check.get('counter') == 1:
        ...
        await ctx.send(f"@{member.name} Toxicity detected after warning!")
        await member.kick()
    elif counter_check is not None and counter_check.get('counter') == 3:
        await ctx.send(f"@{member.name} Repeateted Toxic behavior won't be tolerated!")
        await member.ban()
    else:
        await ctx.send(f"@{member.name} No action taken. Your behavior will be monitored.")
